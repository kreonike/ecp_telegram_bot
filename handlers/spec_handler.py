...
async def get_spec(message: types.Message, state: FSMContext):
    ...
    if question_spec.lower() == 'вернуться в меню':
    ...
    checking_spec = spec_check(spec, base_ecp_medspecoms_id)
    if not checking_spec:
        await message.reply('Неверный ввод специальности, выберите специальность из меню')
        return
    ...
    data_lpu_person = [
        ...
        if item.get('RecType_id') == '1'
           ...
           and item.get('MedStaffFact_id') not in EXCLUDED_DOCTORS  # Используем список исключений врачей
    ]
